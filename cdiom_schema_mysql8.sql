-- ============================================
-- CDIOM医药管理系统数据库脚本 (MySQL 8.0.37优化版)
-- 版本：1.0.3 (专门为MySQL 8.0.37优化)
-- 创建时间：2025-12-27
-- 兼容性：MySQL 8.0.x
-- ============================================
...
CREATE TABLE drug_info (
    ...
    is_special_drug TINYINT UNSIGNED DEFAULT 0 COMMENT '是否特殊药品：0-否，1-是',
    ...
    KEY idx_is_special_drug (is_special_drug),
    KEY idx_status (status),
    KEY idx_create_time (create_time)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
...
CREATE TABLE inventory (
    ...
    warning_quantity INT UNSIGNED DEFAULT 10 COMMENT '预警数量',
    status TINYINT UNSIGNED DEFAULT 1 COMMENT '状态：0-禁用，1-启用',
    create_time DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    update_time DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (id),
...
CREATE TABLE purchase_order (
    ...
    order_date DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3) COMMENT '下单日期',
    ...
    KEY idx_create_time (create_time),
    CONSTRAINT fk_purchase_order_approver FOREIGN KEY (approver_id) REFERENCES sys_user(id)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
  COMMENT='采购订单表';
...
CREATE TABLE purchase_order_item (
    ...
    KEY idx_quantity (quantity),
    KEY idx_create_time (create_time),
    CONSTRAINT fk_purchase_order_item_order FOREIGN KEY (order_id) REFERENCES purchase_order(id),
...
CREATE TABLE outbound_apply (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '申领ID',
    apply_number VARCHAR(50) NOT NULL COMMENT '申领单号',
    applicant_id BIGINT UNSIGNED NOT NULL COMMENT '申领人ID',
    department VARCHAR(100) COMMENT '申领部门',
    apply_date DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3) COMMENT '申领日期',
    reason VARCHAR(200) COMMENT '申领原因',
    urgency_level VARCHAR(20) DEFAULT 'NORMAL' COMMENT '紧急程度：LOW-低，NORMAL-普通，HIGH-高，URGENT-紧急',
    total_quantity INT UNSIGNED COMMENT '总申领数量',
...
CREATE TABLE outbound_apply_item (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '明细ID',
    apply_id BIGINT UNSIGNED NOT NULL COMMENT '申领ID',
    drug_id BIGINT UNSIGNED NOT NULL COMMENT '药品ID',
    apply_quantity INT UNSIGNED NOT NULL COMMENT '申领数量',
    approved_quantity INT UNSIGNED COMMENT '批准数量',
    actual_quantity INT UNSIGNED COMMENT '实际出库数量',
    batch_number VARCHAR(50) COMMENT '批号',
    ...
    KEY idx_drug_id (drug_id),
    KEY idx_batch_number (batch_number),
    KEY idx_apply_quantity (apply_quantity),
    KEY idx_approved_quantity (approved_quantity),
    KEY idx_actual_quantity (actual_quantity),
    KEY idx_create_time (create_time),
    CONSTRAINT fk_outbound_apply_item_apply FOREIGN KEY (apply_id) REFERENCES outbound_apply(id),
...
CREATE TABLE operation_log (
    ...
    operation_time DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3) COMMENT '操作时间',
    ...
    execution_time BIGINT UNSIGNED COMMENT '执行时间（毫秒）',
...
-- 显示创建结果
-- ============================================
SELECT 'MySQL 8.0.37 Database cdiom_db created successfully!' as result;
SHOW TABLES;
